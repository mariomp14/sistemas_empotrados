...
I2CMStateIdle(tI2CMInstance *psInst, tI2CMCommand *pCommand, portBASE_TYPE *higherPriorityTaskWoken)
{
   ...
   /* if(psInst->ui8ReadPtr == psInst->ui8WritePtr)
    ...
    //comprobamos si hay algo en la cola de mensajes//*ojo es fromISR ya que esta funcion se activa dentro del contexto de la rutina de interrupcion
    //si hay algo en la cola de mensajes realiza la transacción
    if(xQueueReceiveFromISR(psInst->I2CcommandsQueue,pCommand,higherPriorityTaskWoken)==pdFALSE)//argumentos,puntero a la cola de mensajes a leer,puntero a donde queremos guardar los datos,
    {                                                        //y en este caso no pasamos el tiempo de bloqueo(tiempo de espera), ya que estamos en una ISR, si no que pasamos
           return;                                                  //por referencia una variable para finalizar luego correctamente la rutina de interrupcion(devuelve true o false
    }                                                              //dependiendo si se despierta una tarea más prioritaria
    //
                  ...
                  uint32_t ui32Status)
{
    ...
    //*Esto era para cuando se había completado un comando se eliminaba del buffer circular,
    //en nuestro caso ya no es necesario porque ya lo hacemos en la moficiacion que hemos hecho
//    psInst->ui8ReadPtr++;
...
//
//*****************************************************************************
/*Es el manejador de interrupción del periférico maestro I2C, que se ejecuta
conforme van ocurriendo eventos relacionados con dicho periférico, y se encarga de tratar dichos
eventos */
void
I2CMIntHandler(tI2CMInstance *psInst)
{
   ...
   // pCommand = &(psInst->pCommands[psInst->ui8ReadPtr]);//*
    pCommand=&psInst->OnGoingI2CTransaction;//guardamos en pCommand un puntero que apunta a la primera posicion de la transaccion
    ...
    while(1)
    {
        ...
        //
        switch(psInst->ui8State)//(máquina de estados)
        {
            //
            // The idle state.
            //
            case STATE_IDLE:
            {
                ...
                //
                I2CMStateIdle(psInst, pCommand,&higherPriorityTaskWoken);//*&higherPriorityTaskWoken

                //
                // This state is done and the next state should be handled at
                // the next interrupt.
                //
                portEND_SWITCHING_ISR(higherPriorityTaskWoken);//*asi es como finalizamos correctamente la rutina de interrupcion
                //lo que significa es que en caso de que despierte una tarea más prioritaria no finalizo la interrupcion haciendo la conmutacion de tareas, ya se
                //hará tras al menos un tic del sistema
                return;
            }
...
//
//*****************************************************************************
//*Esta función se encarga de inicializar la biblioteca y el periférico maestro I2c que le indiquemos
void
         ...
         uint_fast8_t ui8TxDMA, uint_fast8_t ui8RxDMA, uint32_t ui32Clock)
{
    ...
    //Aqui hay que inicializar(crear la cola de mensajes)
     psInst->I2CcommandsQueue=xQueueCreate(NUM_I2CM_COMMANDS,sizeof(tI2CMCommand));//*

     if(psInst->I2CcommandsQueue==0)//si no se consigue crear la cola//*
     {
         while(1);
     }
    ...
    //ojo, hay que dar un nivel de prioridad adecuado, ver niveles de prioridad en FreeRTOSConfig.h
    MAP_IntPrioritySet(ui8Int,configMAX_SYSCALL_INTERRUPT_PRIORITY);//*le paso la interrupcion y su prioridad
...
//
//*****************************************************************************
/*Esta función permite dar la orden de realizar una transacción I2C. Como
parámetros se pasan el manejador del periférico maestro I2C (inicializado con la función anterior),
se le indican los datos a enviar y recibir, y se configura una función “callback”, que será llamada por
la rutina de interrupción del maestro I2C cuando la transacción haya finalizado. En nuestro caso,
esta función se utiliza en los ficheros BMI160.c y ACME3416.c para realizar operaciones I2C de
lectura, escritura o escritura/lectura. */

uint_fast8_t
I2CMCommand(tI2CMInstance *psInst, uint_fast8_t ui8Addr,                  //*realiza el guardado en la cola del nuevo buffer que se estaba creando
            ...
            tSensorCallback *pfnCallback, void *pvCallbackData)
{
   ...
   // tI2CMCommand *pCommand;//*En lugar de crear un puntero a una estructura de tipo pCommand creamos una variable comando
    tI2CMCommand Command;   //*ya que en una cola de mensajes no podemos acceder de esta forma a posiciones de memoria
    ...
    // to the queue).
    //
    //*Comentamos todo de lo que hacia uso del buffer circular que teniamos antes
...
//    pCommand = &(psInst->pCommands[psInst->ui8WritePtr]);
   ...
   //Accedemos a la variables como estructura y no como puntero. Así estaban anteriormente: pCommand->ui8Addr = ui8Addr;
   ...
   // psInst->ui8WritePtr = ui8Next;//*
    //una vez que tenemos rellenada la variable la guardamos en la cola de mensajes-> la recibimos en el estado idle

    xQueueSend(psInst->I2CcommandsQueue,&Command,portMAX_DELAY);//*Los parametros que se le pasan son:manejador de la cola,
                                                                //puntero a la transacción(el comando a enviar), y tiempo que
                                                                //queremos esperar que ese manejador se almacene en la cola de mensajes(en este caso tiempo infinito)

    //ojo, al tener tiempo de espera infito esto hace que la funcion I2CMcommand sea bloqueante y solo pueda llamarse desde una tarea!!!
    //Tener cuidado ya que hay funciones como I2CReadModifyWrite8, llama a I2CMWrite e I2CMRead que a su vez llaman a I2CMcommand, por lo tanto estas funciones solo
    //se pueden ejecutar desde el nivel de tarea//
...
//*****************************************************************************
uint_fast8_t
I2CMTransferResume(tI2CMInstance *psInst, uint8_t *pui8Data)//*esta funcion lo que hace es reanudar una transaccion en curso que se ha detenido
{
    //
    // Check the arguments.
    //
    ...
    if(psInst->ui8State == STATE_WRITE_PAUSE)
    {
        //Ahora la transaccion en curso ya no está en el buffer circular(Que hemos eliminado), ni en nuestra cola de mensajes
        //si no en nuestra variable OnGoingTransaction
        //psInst->pCommands[psInst->ui8ReadPtr].pui8WriteData = pui8Data;//*
        psInst->OnGoingI2CTransaction.pui8WriteData = pui8Data;//*
    }
    else
    {
        //psInst->pCommands[psInst->ui8ReadPtr].pui8ReadData = pui8Data;//*
        psInst->OnGoingI2CTransaction.pui8ReadData = pui8Data;
    }
