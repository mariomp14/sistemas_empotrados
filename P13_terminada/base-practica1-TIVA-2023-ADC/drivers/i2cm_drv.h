...
//librerias que me hacen falta para usar colas y freerto
#include "FreeRTOS.h"//*
#include "queue.h"//*
...
typedef struct
{
    ...
    //
    void *pvCallbackData;//Callback
}
tI2CMCommand;//datos de la transaccion I2C
...
typedef struct
{
    ...
    //
    //uint8_t ui8ReadPtr;//puntero de lectura(Indice) del buffer circular//*
    ...
    //
    //uint8_t ui8WritePtr;//puntero de escritura(Indice) del buffer circular//*
    ...
    //
    //tI2CMCommand pCommands[NUM_I2CM_COMMANDS];//array del buffer circular, de tipo tI2CMCommand(datos de la transaccion)//*
    //comentamos y ejecutamos para que produzca error e ir sustituyendo todo por la cola de mensajes

    //sustituimos el buffer circular por una cola de mensajes+ una variables que me permita guardar la transaccion en curso
    tI2CMCommand OnGoingI2CTransaction;//(transaccion en curso//*
    QueueHandle_t I2CcommandsQueue;//manejador de la cola de mensajes,(añadir librerias necesarias)//*
}
tI2CMInstance;//se crea una por cada puerto I2C
